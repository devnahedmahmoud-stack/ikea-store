...
const FilterdProducts = ({
  ...
  isUnderPrice,
  title,
}: FilterdProductsProps) => {
  const [maxPrice, setMaxPrice] = useState<number>(underPrice);
  const [selectedCategory, setSelectedCategory] = useState<string>("All");
  const categories = [
    "All",
    ...Array.from(new Set(ProductCategories.map((item) => item.name))),
  ];
  console.log(products);
  // Filtered products calculation
  const filteredProducts = useMemo(() => {
    return products.filter((item) => {
      const matchesPrice = isUnderPrice
        ? item.price < maxPrice
        : item.price <= maxPrice;
  ...
  }, [maxPrice, selectedCategory, products, isUnderPrice]);

  return (
    <div className="min-h-screen text-gray-900 lg:p-12 p-6">
      {/* Header */}
      <header className="bg-white border-b border-gray-200 py-6">
        <h1 className="p-8 text-3xl font-bold tracking-tight bg-[#484848] text-white">
          {title}
        </h1>
      </header>
      ...
      <div className="max-w-7xl py-8  lg:flex lg:gap-8">
        ...
        <aside className="w-full lg:w-64 shrink-0 bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit mb-8 lg:mb-0">
          <h2 className="text-sm text-black/70 border-b pb-4 mb-4">
