...
export default function ProductThumbnailSlider({
  ...
  const btn_Next: string = "button-next-thumbnail"    
  const btn_Prev: string = "button-prev-thumbnail"    
  const btn_Prev_Position: string = "left-1/2 top-0 -translate-y-1/2 -translate-x-1/2"    
  const btn_Next_Position: string = "left-1/2 top-full -translate-y-1/2 -translate-x-1/2"    
...
console.log(activeIndex)
 ...
 // console.log(images[currentImageIndex]);
  ...
  return (
    <div
      className={cn(
        " relative w-28 h-screen lg:flex hidden",
        className,
      )}
    >
      <button
      ...
      >
          {images.map((image, index) => (
              ...
              >
                <div
                  className={cn(
                    "relative w-24 h-20 border mb-10 cursor-pointer hover:border-black",
